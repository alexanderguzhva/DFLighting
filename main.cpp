...
const float visibilityEps = 0.00001;
...
void GenerateLinesCombinedEdge()
{
    ...
    for (int i = 0; i < NGenPointsUFEdge; i++)
    {
        ...
        ////
        double dx = 200 * NLayersFaced * x0;
        double dy = 200 * NLayersFaced * y0;
        double dz = 200 * NLayersFaced * z0;
        ...
        std::vector<MyDoubleEqPoint> points;

        //// find all points within 0 <= z <= NLayersEdged that hits x=const, y=const or z=const
        bool bFlag = false;

        //// whether this box is ok. Bad block = intersects lines (2 planes) or points (3 planes)
        bool bSucceeded = true;

        //// one may start from (x0, y0, z0), but i'll start from (0, 0, 0)
        double cx = 0;
        double cy = 0;
        double cz = 0;
        ...
        //GetCurrentBox(cx, cy, cz, &currentBoxX, &currentBoxY, &currentBoxZ);

        ////
        double invx = 1 / fabs(x0);
        double invy = 1 / fabs(y0);
        double invz = 1 / fabs(z0);
        ...
        while (!bFlag)
        {
            //// find least close face to next value
            double minValue = min(min(leftx, lefty), leftz);

            cx += minValue * x0;
            cy += minValue * y0;
            cz += minValue * z0;

            bool gotx = (leftx == minValue);
            bool goty = (lefty == minValue);
            bool gotz = (leftz == minValue);
            ...
            if (gotx || goty || gotz)
            {
                ...
                else
                {
                    ...
                    rintz = (int)(round(cz));
                    rvz = rintz;
                    if (rintz == 2 * NLayersFaced - 1)
                        bFlag = true;
                }

                ////
                int nIntersections = (gotx ? 1 : 0) + (goty ? 1 : 0) + (gotz ? 1 : 0);
                if (nIntersections > 1)
                {
                    bSucceeded = false;
                    bFlag = true;
                ...
                //// add a point
                points.push_back(MyDoubleEqPoint(rvx, rvy, rvz, gotx, goty, gotz));
                ...
                My3DFace * sourceFace = nullptr;
                My3DFace * destFace = nullptr;

                if (gotx)
                {
                    //// left or right
                    int leftsideX = 2 * (currentBoxX - 1) + 1;
                    int rightsideX = leftsideX + 2;

                    if (rintx == leftsideX)
                    {
                        //// go from left to right
                        nextBoxX = currentBoxX - 1;
                        nextBoxY = currentBoxY;
                        nextBoxZ = currentBoxZ;

                        My3DFacedCell * currentCell = MapComputeCellsFaced->operator ()(currentBoxX, currentBoxY, currentBoxZ);
                        sourceFace = currentCell->LeftFace;

                        My3DFacedCell * nextCell = MapComputeCellsFaced->operator ()(nextBoxX, nextBoxY, nextBoxZ);
                        destFace = nextCell->RightFace;
                    }
                    else if (rintx == rightsideX)
                    {
                        ...
                        My3DFacedCell * currentCell = MapComputeCellsFaced->operator ()(currentBoxX, currentBoxY, currentBoxZ);
                        sourceFace = currentCell->RightFace;

                        My3DFacedCell * nextCell = MapComputeCellsFaced->operator ()(nextBoxX, nextBoxY, nextBoxZ);
                        destFace = nextCell->LeftFace;
                    }
                    else
                    {
                        cout << "wtf";
                ...
                {
                    //// top or bottom
                    int leftsideY = 2 * (currentBoxY - 1) + 1;
                    int rightsideY = leftsideY + 2;

                    if (rinty == leftsideY)
                    {
                        //// go from top to bottom
                        nextBoxX = currentBoxX;
                        nextBoxY = currentBoxY - 1;
                        nextBoxZ = currentBoxZ;

                        My3DFacedCell * currentCell = MapComputeCellsFaced->operator ()(currentBoxX, currentBoxY, currentBoxZ);
                        sourceFace = currentCell->TopFace;

                        My3DFacedCell * nextCell = MapComputeCellsFaced->operator ()(nextBoxX, nextBoxY, nextBoxZ);
                        destFace = nextCell->BottomFace;
                    }
                    else if (rinty == rightsideY)
                    {
                        ...
                        My3DFacedCell * currentCell = MapComputeCellsFaced->operator ()(currentBoxX, currentBoxY, currentBoxZ);
                        sourceFace = currentCell->BottomFace;

                        My3DFacedCell * nextCell = MapComputeCellsFaced->operator ()(nextBoxX, nextBoxY, nextBoxZ);
                        destFace = nextCell->TopFace;
                    }
                    else
                    {
                        cout << "wtf";
                ...
                else
                {
                    //// gotz
                    //// up or down
                    int leftsideZ = 2 * (currentBoxZ - 1) + 1;
                    int rightsideZ = leftsideZ + 2;
                    ...
                    if (rintz == leftsideZ)
                    {
                        ...
                        nextBoxX = currentBoxX;
                        nextBoxY = currentBoxY;
                        nextBoxZ = currentBoxZ - 1;

                        My3DFacedCell * currentCell = MapComputeCellsFaced->operator ()(currentBoxX, currentBoxY, currentBoxZ);
                        sourceFace = currentCell->UpFace;

                        My3DFacedCell * nextCell = MapComputeCellsFaced->operator ()(nextBoxX, nextBoxY, nextBoxZ);
                        destFace = nextCell->DownFace;
                    }
                    else if (rintz == rightsideZ)
                    {
                        //// go from down to up
                        nextBoxX = currentBoxX;
                        nextBoxY = currentBoxY;
                        ...
                        My3DFacedCell * currentCell = MapComputeCellsFaced->operator ()(currentBoxX, currentBoxY, currentBoxZ);
                        sourceFace = currentCell->DownFace;

                        My3DFacedCell * nextCell = MapComputeCellsFaced->operator ()(nextBoxX, nextBoxY, nextBoxZ);
                        destFace = nextCell->UpFace;
                    }
                    else
                    {
                        cout << "wtf";
                ...
                currentBoxZ = nextBoxZ;

                ////
                if (sourceFace != nullptr)
                    sourceFace->SourceHits += 1;
                if (destFace != nullptr)
                    destFace->DestinationHits += 1;
            }
            else
            {
                //// nothing happened, go next
                leftx -= minValue;
                lefty -= minValue;
                leftz -= minValue;
            }
        ...
        for(auto point : points)
        {
            ...
            {
                //// XEq
                int yStC = (point.Y > 0) ? (int)(point.Y) : (int)(point.Y - 1);
                int zStC = (point.Z > 0) ? (int)(point.Z) : (int)(point.Z - 1);
            ...
            {
                //// YEq
                int xStC = (point.X > 0) ? (int)(point.X) : (int)(point.X - 1);
                int zStC = (point.Z > 0) ? (int)(point.Z) : (int)(point.Z - 1);
            }
            else
            {
...
void main2()
{
    MapComputeCellsFaced = new MapComputeCellFacedMap;

    GenerateLinesCombinedEdge();
}
